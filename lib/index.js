...
function handleSendEmail(req, res) {
  const transporterOptions = overrideTransporterOptions(req) || config.transporterOptions;
  const { mailOptions } = req.body;
  const { from } = mailOptions;
  // If sender specifies no from, use default
  if (from == null) {
    mailOptions.from = config.from;
  }
  // If sender specifies no bcc, use default
  const { bcc } = mailOptions;
  if (bcc) {
    mailOptions.bcc = config.bcc;
  }
